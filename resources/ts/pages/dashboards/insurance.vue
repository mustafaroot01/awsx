...
<template>
  ...
  <section>
    ...
    <VRow v-if="$can('read', 'Evaluation') && hasActivePeriod" class="mb-6">
      <VCol cols="12">
        <VCard elevation="2" class="rounded-lg" style="background: #ffffff; border-block-start: 3px solid #7367f0;">
          <VCardText class="pa-4">
            <VRow align="center">
              <!-- Title & Status -->
              <VCol cols="12" md="4">
                <div class="d-flex align-center gap-3">
                  <VAvatar color="primary" size="48" rounded="sm" variant="flat">
                    <VIcon icon="tabler-clipboard-check" color="white" size="22" />
                  </VAvatar>
                  <div>
                    <div class="d-flex align-center gap-2 mb-1">
                      <span class="text-h6 font-weight-bold text-high-emphasis">التقييم الدوري</span>
                      <VChip color="primary" variant="tonal" size="x-small" label class="font-weight-bold">
                        مفتوحة
                      </VChip>
                    </div>
                    <div class="text-caption text-medium-emphasis">{{ activePeriodLabel }}</div>
                  </div>
                </div>
              </VCol>
              ...
              <!-- Stats -->
              <VCol cols="12" md="5">
                <div v-if="evalLoading" class="d-flex align-center justify-center gap-2">
                  <VProgressCircular indeterminate color="primary" size="18" width="2" />
                  <span class="text-caption text-medium-emphasis">جاري التحميل...</span>
                </div>

                <div v-else class="d-flex align-center justify-center gap-2">
                  <div class="text-center px-3 py-2 rounded-sm flex-grow-1" style="background: #f8f7ff; border: 1px solid rgba(115,103,240,0.15);">
                    <div class="text-h5 font-weight-bold text-high-emphasis">{{ evalProgress.evaluated }}</div>
                    <div class="text-caption text-medium-emphasis">مُقيَّم</div>
                  </div>
                  <div class="text-center px-3 py-2 rounded-sm flex-grow-1" style="background: #f8f7ff; border: 1px solid rgba(115,103,240,0.15);">
                    <div class="text-h5 font-weight-bold text-high-emphasis">{{ evalProgress.total - evalProgress.evaluated }}</div>
                    <div class="text-caption text-medium-emphasis">متبقي</div>
                  </div>
                  <div class="text-center px-3 py-2 rounded-sm flex-grow-1" style="background: #f8f7ff; border: 1px solid rgba(115,103,240,0.15);">
                    <div class="text-h5 font-weight-bold text-primary">{{ evalProgress.pct }}%</div>
                    <div class="text-caption text-medium-emphasis">إنجاز</div>
                  </div>
                </div>
              </VCol>

              <!-- Action -->
              <VCol cols="12" md="3" class="text-md-end mt-3 mt-md-0">
                <VBtn
                  color="primary"
                  variant="elevated"
                  size="large"
            ...
            <!-- Progress -->
            <div v-if="!evalLoading" class="mt-4">
              <div class="d-flex align-center justify-space-between mb-1">
                <span class="text-caption text-medium-emphasis">نسبة الإنجاز</span>
                <span class="text-caption font-weight-bold text-warning">{{ evalProgress.pct }}%</span>
              </div>
              <VProgressLinear
              ...
              >
                ...
                <VListItemSubtitle class="d-flex align-center gap-2">
                  ...
                  <VChip
                    ...
                    :color="emp.type === 'admin_producer' ? 'info' : 'success'"
