...
export const canNavigate = (to: RouteLocationNormalized) => {
  ...
  if (targetRoute?.meta?.action && targetRoute?.meta?.subject)
    return ability.can(targetRoute.meta.action as any, targetRoute.meta.subject as any)

  // If no specific permissions, fall back to checking if any parent route allows access
  return to.matched.some(route => {
      if (route.meta && route.meta.action && route.meta.subject) {
          return ability.can(route.meta.action as any, route.meta.subject as any)
      }
      return true
  })
}
